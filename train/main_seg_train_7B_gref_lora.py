...
class QwenVLSFTTrainer:
    def __init__(self, base_model_name, lora_model_name: str, output_dir: str):
        self.model_name = lora_model_name
        ...
        self.processor = AutoProcessor.from_pretrained(base_model_name, trust_remote_code=True, min_pixels=min_pixels, max_pixels=max_pixels)
        ...
        self.model = PeftModel.from_pretrained(self.model, self.model_name, is_trainable=True)
    ...
    def _create_dummy_dataset(self):
        ...
        base_path = '/efficient_sag4text/playground/data/json_files/'
        json_files = [
            'grefcoco_doubled_mp_fullmask.json',
        ]
        # 构建完整的文件路径
    ...
    def train(self):
        ...
        #     task_type="CAUSAL_LM",
        # )
        # 对于已经通过 PeftModel 加载 LoRA 的模型，这里不需要再做任何操作

        # --- 5. 配置训练参数 ---
        training_args = SFTConfig(
            output_dir=self.output_dir,
            num_train_epochs=2,  # 训练轮数
            per_device_train_batch_size=12,  # 每个设备的批处理大小
...
if __name__ == '__main__':
    ...
    # model_name = '/efficient_sag4text/new_train_qwen_2b/checkpoint-6000/'  # or your local model path
    model_name = 'JiaZL/seg-7B'
    trainer = QwenVLSFTTrainer(
        base_model_name='Qwen/Qwen2-VL-7B-Instruct',
        lora_model_name=model_name,
        output_dir="/apdcephfs_nj4/share_300377003/realzliu/new_train_qwen_7b_gref/",
    )
    trainer.train()